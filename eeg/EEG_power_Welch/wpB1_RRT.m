...
%     Sessions = 'RRT';
    ...
    TitleTag = ['Tasks_', Scaling, '_', plotChannelsLabels];
...
for Indx_T = 1:numel(Tasks)
    ...
    for Indx_S = 1:numel(Sessions)
     ...
     ylim(YLims)
     Matrix = squeeze(nanmean(allFFT(:, Indx_T, :, ChanIndx, :), 4));
    Matrix = permute(Matrix, [1, 3, 2]);
     TimeSeriesStats(Matrix, Freqs, 200);
    legend(SessionLabels)
    ylim(YLims)
    xlim(XLims)
    xlabel('Frequency (Hz)')
...
for Indx_T = 1:numel(Tasks)
    ...
    saveas(gcf,fullfile(Paths.Figures, [TitleTag, '_PowerTopoplots_', Tasks{Indx_T}, '.svg']))
end
