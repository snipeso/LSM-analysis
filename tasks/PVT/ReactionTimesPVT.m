...
Destination = fullfile(Paths.Preprocessed, 'Statistics', Analysis);
    ...
    Results = fullfile(Paths.Results, Analysis);
...
for Indx_C = 1:numel(Conditions)
  ...
  Sessions = Format.Labels.(Task).(Condition).Sessions;
    ...
    ylim([0.1, 1])
    set(gca, 'FontSize', 12)
    saveas(gcf,fullfile(Results, [TitleTag, '_RTs_Flames.jpg']))
    ...
    ylim([-5 5])
    set(gca, 'FontSize', 12)
    saveas(gcf,fullfile(Results, [TitleTag, '_speed_Flames.jpg']))
    
    
    %plot means
    figure
    PlotConfettiSpaghetti(MeanRTs,  SessionLabels, [0.2, 0.6], [], [], Format, PlotStats)
    title([replace(TitleTag, '_', ' '), ' Reaction Time Means'])
    ylabel('RT (s)')
    set(gca, 'FontSize', 12)
    saveas(gcf,fullfile(Results, [TitleTag, '_meanRTs.svg']))
    ...
    figure
    PlotConfettiSpaghetti(stdRTs,  SessionLabels, [0 .2], [],[], Format, PlotStats)
    title([replace(TitleTag, '_', ' '), 'Reaction Time Standard Deviations'])
    ylabel('RT SD (s)')
    set(gca, 'FontSize', 12)
    saveas(gcf,fullfile(Results, [TitleTag, '_stdRTs.svg']))
    ...
    %plot medians
    figure
    PlotConfettiSpaghetti(MedianRTs,  SessionLabels, [0.2, 0.6], [],[], Format, PlotStats)
    ylabel('RT (s)')
    title([replace(TitleTag, '_', ' '),'Reaction Time Medians'])
    set(gca, 'FontSize', 12)
    saveas(gcf,fullfile(Results, [TitleTag, '_medianRTs.svg']))
 ...
 % save matrix
    ...
    figure
    PlotConfettiSpaghetti(Q1Q4,  SessionLabels, [0, 0.2], [],[], Format, PlotStats)
    ylabel('RT (s)')
    title([replace(TitleTag, '_', ' '),' Interquartile Range'])
    set(gca, 'FontSize', 12)
    saveas(gcf,fullfile(Results, [TitleTag, '_InterQRange.svg']))
   ...
   % save matrix
    ...
    figure
    subplot(2, 2, 1)
    PlotConfettiSpaghetti(Top10,  SessionLabels, [0.1, 0.5], [],[], Format, PlotStats)
    ylabel('RT (s)')
    title([replace(TitleTag, '_', ' '),' Fastest 10%'])
    set(gca, 'FontSize', 12)
    axis square
    saveas(gcf,fullfile(Results, [TitleTag, '_Top10.svg']))
    ...
    subplot(2, 2, 2)
    PlotConfettiSpaghetti(Bottom10,  SessionLabels, [0.2, 2], [],[], Format, PlotStats)
    ylabel('RT (s)')
    title([replace(TitleTag, '_', ' '),' Slowest 10%'])
    set(gca, 'FontSize', 12)
    axis square
    saveas(gcf,fullfile(Results, [TitleTag, '_Bottom10.svg']))
  ...
  % save matrix
    ...
    save(fullfile(Destination, Filename), 'Matrix', 'Sessions', 'SessionLabels')
        ...
        subplot(2, 2, 3)
    PlotConfettiSpaghetti(Top20,  SessionLabels, [0.1, 0.5], [],[], Format, PlotStats)
    ylabel('RT (s)')
    title([replace(TitleTag, '_', ' '),' Fastest 20%'])
    set(gca, 'FontSize', 12)
    axis square
    saveas(gcf,fullfile(Results, [TitleTag, '_Top20.svg']))
    ...
    save(fullfile(Destination, Filename), 'Matrix', 'Sessions', 'SessionLabels')
        ...
        subplot(2, 2, 4)
    PlotConfettiSpaghetti(Bottom20,  SessionLabels, [0.2, 2], [],[], Format, PlotStats)
    ylabel('RT (s)')
    title([replace(TitleTag, '_', ' '),' Slowest 20%'])
    set(gca, 'FontSize', 12)
    axis square
    saveas(gcf,fullfile(Results, [TitleTag, '_Bottom20.svg']))
